...
IAR_D0 <- registrosD0 %>%  #Base de datos de registros
  ...
  summarise(IAR= (n_events/n_nights_active)*100) # Aplicamos la formula para cálcular el RAI y listo
```
...
```{r}
# estaría bien darle un vistazo espacial a los registros de la especie.

ggplot(sp_lmdata, aes(x= utm_x, y= utm_y, col= IAR_D15))+
  geom_point(aes(size= IAR_D15))+
  scale_colour_gradient(low= "black", high = "red")+
  scale_size(range = c(3,15))+
  theme_bw()
```

Al tomar el IAR valores continuos suponemos que seguirá un error tipo *gaussiano*, por lo cual usaremos modelos lineales simples para modelarlo. NOTA: Al no tomar valores negativos quizás pueda ser error tipo gamma pero depende de cada caso probarlo.
